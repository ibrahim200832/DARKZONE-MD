// Wires the anti-link, anti-spam, self-update, group-invite, auto-reply and
// AI-chat handlers up to a Baileys socket's incoming-message stream. See
// example-connect.js for a runnable demo.
const { handleAntiLink } = require('./antilink');
const { handleAntiSpam } = require('./antispam');
const { handleUpdateCommand, startUpdateWatcher } = require('./selfupdate');
    ...
    sock.ev.on('messages.upsert', async ({ messages, type }) => {
        ...
        for (const msg of messages) {
            ...
            try {
                ...
                const updateHandled = await handleUpdateCommand(sock, msg, opts);
                if (updateHandled) continue;
