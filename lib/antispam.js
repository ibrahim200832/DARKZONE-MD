...
// How long a sender can go quiet before an earlier warning is forgotten and
// stale tracking data for them is dropped, so a warning from one long-past
// burst doesn't follow someone around indefinitely (and memory doesn't grow
// forever for senders who only ever appear once).
const IDLE_RESET_MS = INTERVAL_MS * 5;
...
function recordMessage(jid, sender) {
    ...
    const lastSeen = entry?.timestamps[entry.timestamps.length - 1] ?? 0;
    if (!entry || now - lastSeen > IDLE_RESET_MS) {
